...
ELEVEN tables across TEN addons: `emberwatch/rules.json`, `ledgerline/floors.json`, `longwatch/mobs.json`, `longwatch/rares.json`, `lorebind/items.json`, `purelight/refused.json`, `satchel/bags.json`, `tocsin/bosses.json`, `trailmark/quests.json`, `veinsight/nodes.json`, `wayfarer/atlas.json`. This said "nine across eight" and omitted `purelight` and `satchel` for two cycles; `ls addons/*/generate.mjs | wc -l` is the whole check and `pnpm tables` names every output it wrote, so either one catches it.

**The flag forms disagree.** Five take `--game=<path>` only (`longwatch`, `lorebind`, `trailmark`, `veinsight`, `wayfarer`); five accept both (`emberwatch`, `ledgerline`, `purelight`, `satchel`, `tocsin`). **`pnpm tables` reports a generator that broke for ANY reason as a flag error**, because it retries both spellings and surfaces the last failure: at game 0.41.0 `veinsight` reported `no --game=<path>` when the real fault was a game export that had moved. Run the generator directly before believing the flag. Passing the wrong one trips the required-argument error. `pnpm tables --game <checkout>` tries both spellings per generator and classifies every table, which is why it should be what runs them rather than eight invocations by hand. Measured again at game 0.41.4 by RUNNING all twenty invocations: the restrictive five are unchanged and the permissive side grew from three to five, `purelight` and `satchel` having landed since. The 0.40.1 reading before it caught a different error the same way, having named `wayfarer` among the permissive ones when it is not; reading the sources rather than running them is what produced that, since the parsers do not share an idiom (`emberwatch` matches the inline form with a regex rather than `startsWith`, and the refusal wording differs per generator, so grepping for one phrase misclassifies the rest).
