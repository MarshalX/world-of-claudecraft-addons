...
|---|---|---|---|
| HUD anchors | `index.html`, `play.html`, `src/ui/options_window.ts`, `src/ui/interface_unlock.ts` | Resolve each selector in `loader/src/runtime/ui/anchors.ts` against the game's markup. There are ELEVEN, not the eight this said for several cycles: `#ui`, `#options-menu`, `.opt-list`, `.opt-version`, `[data-back]`, `#side-buttons-col-b`, `#mm-options`, `#zone-label`, `#game-version`, and `body.interface-unlocked`. Read the count off `ANCHORS` rather than from here. | A moved selector is an injection point that silently stops existing. `anchors.ts` is one table so the repair is one edit, and the manager's Diagnostics pane resolves them live. |
