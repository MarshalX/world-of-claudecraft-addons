...
Game 0.42.0 added the `self` branch, and it is the first time the server has sent a combat flag for YOU rather than for a party row. That is why the branch sits above the other four, and why it is read only when it says true: the field it comes from is one the client defaults to false, so a false cannot be told apart from a server that never sent one, and treating it as an answer would reinstate the bug below on any older server.

There IS an `inCombat` field on the entity, and it is now written for your own record and nothing else. On every mob, npc and other player it is still never written and still reads false forever, which is why this API does not hand it to you: `world.combat` is the reading, and `source === 'self'` is how you tell that the server itself answered.
