...
const COUNTS = [COUNT_APPLICATIONS, COUNT_SOAKERS];
/** For a fraction of maximum health read off the wire and drawn as a percentage. */
const PERCENT = 100;
...
function optionalOf(value) {
  return {
    ...
    counts: countsOr(value.counts),
  };
}

/**
 * An unrecognised value falls back to applications, never to soakers: a typo must not change
 * what a badge MEANS.
 */
function countsOr(value) {
  if (typeof value === 'string' && COUNTS.includes(value)) {
    return value;
  }
...
function soakersOf(rule, aura) {
  ...
  return aura.stacks ?? null;
}

/** `value2` on a soak is the whole hit, before it is divided by whoever turns up. */
function shareOf(rule, aura) {
  if (rule.counts !== COUNT_SOAKERS || typeof aura.value2 !== 'number') {
    return null;
  }
  return aura.value2;
}
...
function fromAura(rule, unit, unitKey, aura) {
  return {
    ...
    soakers: soakersOf(rule, aura),
    share: shareOf(rule, aura),
...
function fromRow(rule, member, row) {
  ...
  return {
    ...
    school: null,
    stacks: null,
    // A party row carries no stack count, so a soak rule pointed at one has nothing to draw.
    soakers: null,
    share: null,
...
/**
 * The badge in the corner of a square. A soak's figure is drawn even at one, where a single
 * application is not worth a badge.
 */
function stackCount(found) {
  if (found.rule.counts === COUNT_SOAKERS) {
    return found.soakers;
  }
...
/** What a soak's whole hit is, before the people who turn up divide it between them. */
function shareLines(found) {
  if (found.share === null) {
    return [];
  }
  const share = `${String(Math.round(found.share * PERCENT))}%`;
  return [{ text: `They divide ${share} of one player's maximum health.`, tone: 'muted' }];
}

/**
 * What a soak row's badge counts, which the badge itself cannot say: the words are the only
 * place the difference from applications exists.
 */
function soakLines(found) {
  if (found.rule.counts !== COUNT_SOAKERS) {
    return [];
  }
  if (found.soakers === null) {
    return [{ text: 'How many have to stand in it did not reach this reading.', tone: 'warn' }];
  }
  // Phrased so the sentence agrees with "1 player" as well as "4 players".
  const needed = woc.fmt.count(found.soakers, 'player');
  return [
    { text: `Needs ${needed} standing in it.`, tone: 'danger' },
    ...shareLines(found),
    {
      text: 'Only whoever is marked sees this: an aura on another raider is unreadable.',
      tone: 'warn',
    },
  ];
}
...
/** The soak count for the accessible name, since a bare badge cannot say what it counts. */
function saidCount(found) {
  if (found.rule.counts !== COUNT_SOAKERS) {
    return '';
  }
  if (found.soakers === null) {
    return ', and how many have to stand in it is not on this reading';
  }
  return `, needing ${woc.fmt.count(found.soakers, 'player')}`;
}

/** A tile is all art, so everything it says has to be in its accessible name too, the face included. */
function labelFor(found) {
  const said = `${found.rule.label}: ${found.name} on ${found.who}${saidCount(found)}`;
  if (found.castBy === null) {
    return said;
  }
...
function describe(rule) {
  ...
  if (rule.counts === COUNT_SOAKERS) {
    parts.push('counting soakers, not applications');
  }
