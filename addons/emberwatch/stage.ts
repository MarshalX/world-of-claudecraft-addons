...
const ALERTS_ALT =
  "a row of five square alert tiles above the rules panel that produced them. Left to right: Silencing Shriek at 3 seconds, bordered in shadow purple with no picture in it at all, because a mob applied it and skill art is filed under a player class; Brood Venom at 9 seconds in nature green, a 3 in its corner for the three bites that have landed, and artless for a second reason, that its aura id is not an ability id and no file is named for it; Aether Surge at 3 seconds, drawn in the warning amber a rule watching for an effect running out puts over the school colour, and artless for a third reason, that the game paints that icon at run time from a module an addon cannot reach; Hot Streak at 7 seconds in fire red; and Pyrelance at 4 seconds, amber again. The last two carry the art the game ships for the ability that applied them. Each tile names who is carrying the effect underneath: Marshal four times, and The Broodmother, clipped to the width of the square, under the mage's own dot on the boss. Below the strip, the Emberwatch rules panel lists the eight rules in force for a mage, each a switched-on checkbox with what it fires on written beside it: four that apply to every class, watching for a target made untouchable in a duel, for being silenced, for anything harmful reaching three stacks on you, and for being stunned; then Aether Surge ending, Brain Freeze, Hot Streak, and Pyrelance fading on your target, that last one marked mine only. Two lines in smaller type close the panel with what the addon cannot answer: a party row carries no source, no duration and no stacks, and nothing anywhere can say how much damage a control will take before it breaks.";
