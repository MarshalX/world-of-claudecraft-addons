...
// would silently miss those. The two derivations are called rather than copied
...
//
// WHAT IT EXTRACTS is what the game's own item tooltip draws, which is the whole
// point of the table: an addon that can only NAME an item is a lookup box, and a
// player opens an item browser to compare two helmets. So a row carries the
// stats, the ratings, the weapon's damage and speed, the armor class, what a
// consumable restores, what a bag holds, the set it belongs to, whether it is
// soulbound, the two derived levels, and the vendor's price.
//
...
// the heroic variant's base id, the mount key, weapon procs and set BONUSES.
...
//
//   Quality and slot being OPTIONAL. 96 items declare no quality and 281 no slot
//   at game 0.34.0, and the absence is meaningful rather than a gap to fill in.
...
/** The two pure leaves behind the numbers the game derives. Bundled with the table. */
...
/**
 * The game's `ITEMS`, its set table, and the two derivations, all imported for real.
 *
 * Three bundles rather than one because they are three entry points of the game's own; each
...
async function readGame(gamePath) {
  ...
  return { items, sets, itemLevel: level.itemLevel, requiredLevelFor: levelReq.requiredLevelFor };
}
...
function render(gameVersion, provenance, rows) {
  const fields =
    "what the game's own item tooltip draws: id, name and kind always, then quality, slot, " +
    'armorType, itemLevel, stats, spellPower, critRating, hasteRating, hitRating, weapon, ' +
    'blockValue, foodHp, drinkMana, potionHp, potionMana, elixir, bagSlots, requiredLevel, ' +
    'requiredClass, set, soulbound and sellValue wherever the game declares or derives one';
