...
- **zod must never reach the runtime bundle.** `loader/src/shared/schema.ts` is the only zod dependant. The runtime imports its types with `import type`, which `verbatimModuleSyntax` erases. `loader/build-runtime.mjs` fails the build if zod resolves there, and that guard has already earned its place: M5 wired the addon lifecycle, which made `runtime/api/index.ts` reachable from the entry for the first time, and the single `API_VERSION` it imported as a VALUE from `schema.ts` pulled zod and its fifty locale files into the page bundle. `API_VERSION` now lives alone in `shared/api-version.ts` for that reason. A value import of anything in a zod module drags the library; a type import costs nothing.
